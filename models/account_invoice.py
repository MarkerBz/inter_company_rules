...
class account_invoice(models.Model):
    ...
    def inter_company_create_invoice(self, company, inv_type, journal_type):
        ...
        if not self.env['account.invoice'].sudo(intercompany_uid).check_access_rights('create', raise_exception=False):
    ...
    def _prepare_invoice_data(self, inv_type, journal_type, company):
        ...
        vals = {
            'name': self.name,
            'origin': '%s, Invoice: %s' % (self.company_id.name, self.number),
            'type': inv_type,
